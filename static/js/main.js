...
//var words_list = ["APPLE","BALL","CAT","DOG","EAGLE","FISH"];
...
$(document).ready(function () {
  ...
  $(".left").append(
    "<div class='gameboard' style='width:" +
      y_factor +
      ";height:" +
      y_factor +
      ";'></div>"
...
var drawboard = function () {
  ...
  for (var i = 0; i < size; i++) {
    $(".letters").append(
      "<span class='" + (i + 1) + "'>" + getRandomLetter() + "</span>"
    );
    //' ' +"</span>");
  }
  ...
  while (stopper == 0 && autostop < attemptnum) {
    ...
    if (direction === 0) {
      ...
      if (fits <= no_of_rows) {
        ...
        var place = cellxy;
        while (i <= wordlength) {
          if (cell[place] && cell[place] != theword.charAt(i - 1)) coltest = 1;
          place++;
          i++;
        }
        ...
        if (!coltest) {
          ...
          while (i <= wordlength) {
            ...
            if (!grayed[wordnum]) grayed[wordnum] = place;
            else grayed[wordnum] = grayed[wordnum] + "," + place;
            place++;
            i++;
          }
          stopper = 1;
...
function checkWord() {
  ...
  if (
    pos.some(function (o) {
      return (
        o.start === click.startPos && o.end === click.endPos && clearPos(o)
      );
    })
  ) {
    return true;
  ...
  // if user highlights from last letter to first
    ...
    pos.some(function (o) {
      return (
        o.start === click.endPos && o.end === click.startPos && clearPos(o)
      );
    })
  ) {
    return true;
...
function scratchWord() {
  for (var i = 0; i < words.length; i++) {
    if (
      (click.startPos === words[i].start && click.endPos === words[i].end) ||
